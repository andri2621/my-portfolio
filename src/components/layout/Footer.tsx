...
import { CustomLink, Tooltip, UnstyledLink } from '@/components';
    ...
    <>
      ...
      <div className='layout'>
        <footer className='footer footer-center text-base-content py-6'>
          <nav className='flex flex-wrap justify-center gap-x-8 gap-y-4'></nav>
          {footerLink.map((item) => {
            if (item.underConstruction) {
              return (
                <div key={item.name} className='flex cursor-not-allowed gap-2'>
                  {item.label}
